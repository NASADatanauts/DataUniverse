...
# There are around 31k NASA data sets
# This pulls [limit] number of data sets
limit = 100
json_file = 'http://api.us.socrata.com/api/catalog/v1?q=nasa&domains=data.nasa.gov&offset=0&limit=' + \
    str(limit)
...
for x in range(0, limit):
    # +';'+data['results'][x]['permalink']+ ';'+data['results'][x]['resource']['description']+ ';'+data['results'][x]['resource']['name']
    tags = (', '.join(data['results'][x]['classification']['domain_tags']))
    stringtowrite = tags + '\n'
